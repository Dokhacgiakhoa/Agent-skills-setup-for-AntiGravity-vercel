...
export function Navbar() {
  ...
  const navItems = [
    { name: t('navbar.home'), href: "/", color: "#6F00FF" }, // Neon Indigo
    { name: t('navbar.install'), href: "/tutorial", color: "#6F00FF" }, // Neon Indigo
    { 
      name: t('navbar.guide'), 
      href: "/guide",
      color: "#6F00FF", // Neon Indigo
      children: [
  ...
  // Get active color or default to neon indigo if none
  const activeColor = activeIndex !== -1 ? navItems[activeIndex].color : '#6F00FF';
    ...
    <div className="fixed top-8 left-0 right-0 z-50 flex justify-center px-10 pointer-events-none">
      <nav 
        ...
        className={cn(
          ...
          scrolled && "bg-black/40 border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.8),0_0_20px_rgba(111,0,255,0.1)]"
        )}
      >
        <div className="flex w-full items-center justify-between relative px-2">
          <Link href="/" className="flex items-center gap-3 group">
            <div className="w-11 h-11 relative group-hover:scale-110 transition-transform duration-300 aspect-square rounded-xl overflow-hidden shadow-lg shadow-[#6F00FF]/20 border border-white/10 bg-black/40">
              <img 
            ...
            <span className="hidden lg:inline-block font-black text-[#6F00FF] text-sm">
              Agent Skills Setup
            </span>
          </Link>
          ...
          <div className="flex items-center gap-3 z-20 flex-shrink-0">
            ...
            >
               <Globe className="h-4 w-4 text-white/80 group-hover:text-[#6F00FF] transition-colors" />
            ...
            <div className="hidden md:flex items-center gap-2 bg-[#6F00FF]/10 px-3 py-1.5 rounded-full border border-[#6F00FF]/30">
              <div className="w-1.5 h-1.5 bg-[#6F00FF] rounded-full animate-pulse" />
              <span className="text-[9px] font-black text-[#6F00FF] uppercase tracking-widest">{t('common.version')}</span>
            </div>
                    ...
                    <div className="flex items-center gap-2">
                      <Link
                         ...
                         className={cn(
                           ...
                           isActive ? "bg-[#6F00FF] text-white" : "bg-white/5 text-white"
                         )}
                      ...
                      {hasChildren && isExpanded && (
                        ...
                        >
                          {item.children!.map((child) => (
                             ...
                             >
                                 <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#6F00FF]">
